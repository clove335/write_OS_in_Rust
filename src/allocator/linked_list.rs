...
impl LinkedListAllocator {
    ...
    // add the given memory to the front of the list
    ...
    //
    // Returns the allocation start addressa on success
    fn alloc_from_region(region: &ListNode, size: usize, align: usize) -> Result<usize, ()> {
        ...
        if excess_size > 0 && excess_size < mem::size_of::<ListNode>() {
            // Rest of region is too small to hold a ListNode
            // (required because the allocation splits the region in a used and a free part)
            return Err(());
        }

        // Region that is suitable for allocation
        Ok(alloc_start)
    }
